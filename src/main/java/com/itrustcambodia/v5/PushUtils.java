...
public final class PushUtils {
    ...
    public static final long schedule(JdbcTemplate jdbcTemplate, Long userId, List<Long> countries, List<Long> cities, List<Long> applications, List<Long> platforms, List<Long> manufactures, List<Long> models, List<Long> versions, String message, Date when) {
        ...
        select.append("select device.* from " + TableUtilities.getTableName(Device.class) + " device inner join " + TableUtilities.getTableName(Application.class) + " application on device." + Device.APPLICATION_ID + " = application." + com.itrustcambodia.v5.entity.Application.ID);
        ...
        wheres.add("application." + com.itrustcambodia.v5.entity.Application.USER_ID + " = :userId");
